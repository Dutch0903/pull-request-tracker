...
class UpdateTokenTest {
    ...
    void execute_shouldFetchTokenInfoAndSave() throws TokenPersistenceException {
        ...
        when(tokenInfoProvider.fetch(existingToken.platform(), TokenValue.from(VALUE))).thenReturn(tokenInfo);

        updateToken.execute(ID, tokenName, tokenValue);

        verify(tokenInfoProvider).fetch(existingToken.platform(), TokenValue.from(VALUE));
    ...
    void execute_whenNameHasNotChanged_shouldNotValidateNameUniqueness() {
        ...
        when(tokenInfoProvider.fetch(token.platform(), tokenValue)).thenReturn(tokenInfo);
    ...
    @Test
    void execute_whenNameHasChangedAndAlreadyExists_shouldThrowUpdateTokenException() throws TokenPersistenceException {
        TokenName oldTokenName = TokenName.from("random");
        TokenName tokenName = TokenName.from(NAME);
        ...
        when(tokenRepository.findById(ID)).thenReturn(Optional.of(token));
        when(tokenRepository.existsByName(tokenName)).thenReturn(true);

        UpdateTokenException exception = assertThrows(UpdateTokenException.class,
                () -> updateToken.execute(ID, tokenName, tokenValue));

        assertEquals("Token already exists with name: " + tokenName.value(), exception.getMessage());
