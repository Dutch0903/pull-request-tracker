...
public record CodeRepositoryProjection(String identifier, String owner, String name, String url,
        CodeRepositoryStatus status) {
    public static CodeRepositoryProjection from(CodeRepository repository) {
        return new CodeRepositoryProjection(repository.getIdentifier().value(), repository.getOwner(),
                repository.getName(), repository.getUrl(), repository.getStatus());
    }
}
