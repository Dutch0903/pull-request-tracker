...
public class CodeRepositoryMapper {
    ...
    public CodeRepository toDomain(CodeRepositoryDto dto) {
        return new CodeRepository(CodeRepositoryId.from(dto.id()), new FullName(dto.owner(), dto.name()),
                CodeRepositoryStatus.valueOf(dto.status()), dto.tokenId() != null ? TokenId.from(dto.tokenId()) : null,
                dto.lastCheckedAt() != null ? Instant.parse(dto.lastCheckedAt()) : null);
    }
}
