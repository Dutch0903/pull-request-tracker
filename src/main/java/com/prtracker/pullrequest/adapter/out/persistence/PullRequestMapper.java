...
public class PullRequestMapper {
    ...
    public PullRequestDto toDto(PullRequest pullRequest) {
        List<ReviewDto> reviewDtos = pullRequest.getReviews().stream()
                .map(r -> new ReviewDto(r.reviewer(), r.state().name(), r.submittedAt()))
                .toList();

        return new PullRequestDto(
                pullRequest.getId().value(),
                pullRequest.getCodeRepositoryId().value(),
                pullRequest.getExternalId(),
                pullRequest.getTitle(),
                pullRequest.getAuthor(),
                pullRequest.isDraft(),
                pullRequest.getStatus().name(),
                pullRequest.getCiStatus().name(),
                pullRequest.getCommentCount(),
                List.copyOf(pullRequest.getLabels()),
                List.copyOf(pullRequest.getRequestedReviewers()),
                reviewDtos,
                pullRequest.getCreatedAt(),
                pullRequest.getUpdatedAt(),
                pullRequest.getMergedBy(),
    ...
    public PullRequest toDomain(PullRequestDto dto) {
        List<Review> reviews = dto.reviews().stream()
                .map(r -> new Review(r.reviewer(), ReviewState.valueOf(r.state()), r.submittedAt()))
                .toList();

        return pullRequestFactory.reconstitute(
                PullRequestId.from(dto.id()),
                CodeRepositoryId.from(dto.codeRepositoryId()),
                dto.externalId(),
                dto.title(),
                dto.author(),
                dto.draft(),
                PullRequestStatus.valueOf(dto.status()),
                CiStatus.valueOf(dto.ciStatus()),
                dto.commentCount(),
                dto.labels(),
